"use strict";
// Transcrypt'ed from Python, 2017-12-25 20:36:23
function edit-box () {
    ...
    */
	...
	(function () {
		...
		var cambiar = function (evt) {
			...
			var valor = out.value;
			var imagen = '';
			var __iterable0__ = enumerate (out.py_get (__kwargtrans__ ({py_selector: 'option'})));
			for (var __index0__ = 0; __index0__ < len (__iterable0__); __index0__++) {
			...
			$ (out.parent.py_get (__kwargtrans__ ({py_selector: 'img'})) [0]).removeClass ('hidden');
			var url = ((((config.base_url + '/apps/') + rest ['app']) + '/admin/static/archivos/') + ($ (out.parentNode).find ('img') [0].getAttribute ('archivos').strip () == '' ? 'Imagenes' : $ (out.parentNode).find (__kwargtrans__ ({py_selector: 'img'})) [0].getAttribute ('archivos').strip ())) + '/';
			$.get(url+thumbails(imagen)).done(funcion(){
		...
		var componentes = dict ({'widget-campo-box': Component ('widget-campo-box', dict ({}), true, true), 'widget-campo': Component ('widget-campo', dict ({}), true, true), 'widget-campo-boxes': Component ('widget-campo-boxes', dict ({}), true, true)});
		var upload_image = function (evt) {
			...
			var funcion = function (evt, img) {
				if (typeof img == 'undefined' || (img != null && img .hasOwnProperty ("__kwargtrans__"))) {;
					var img = $ (evt.target.parentNode).find ('img') [0];
				};
				$ (img).remove ('hidden');
		...
		var nuevaSeccion = function (evt) {
			...
			if (componentes ['widget-campo'] != null && ultimo != null) {
				$ ('#custom .tab') [0].children [ultimo].innerHTML = componentes ['widget-campo'].run (data);
			}
			agregar = false;
