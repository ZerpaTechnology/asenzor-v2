"use strict";
// Transcrypt'ed from Python, 2018-01-24 20:47:23
function divi () {
	...
	__nest__ (
		...
		'TinyMCE', {
			__all__: {
				__inited__: false,
				__init__: function (__all__) {
					var __name__ = 'TinyMCE';
					var Widget = __init__ (__world__.Widget).Widget;
					var config = Config.Config ();
					var TinyMCE = __class__ ('TinyMCE', [Widget], {
						__module__: __name__,
						get __init__ () {return __get__ (this, function (self, titulo) {
							Widget.__init__ (self, titulo);
							self._html = '\n\t\t<h5>{}</h5>\n\t\t<textarea>{}</textarea>\n\t\t';
							self.editor = null;
							self.value = '';
							self._load_js = list ([config.base_url + 'static/js/tinymce/js/tinymce/tinymce.min.js', config.base_url + 'static/js/tinymce/js/tinymce/jquery.tinymce.min.js']);
							self.styles = list ([]);
						...
						get reconectar () {return __get__ (this, function (self, timout) {
							if (typeof timout == 'undefined' || (timout != null && timout .hasOwnProperty ("__kwargtrans__"))) {;
								var timout = 0;
							};
							var cargar = function () {
								self.target.find ('>textarea').tinymce (dict ({'language': self.lang, 'theme': self.theme, 'plugins': self.plugins, 'codesample_languages': self.code_langs, 'content_css': self.content_css, 'fontsize_formats': self.fontsize_formats, 'font_formats': self.font_formats}));
							};
						...
						get valueCacher () {return __get__ (this, function (self) {
						...
						get py_update () {return __get__ (this, function (self) {
							self.format = list ([self._titulo, self.value]);
							self.__update__ ();
						});}
					});
					__pragma__ ('<use>' +
