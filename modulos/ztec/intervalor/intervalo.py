...
def getConjuntos(cadena,etiquetas,exclusion=None):
	...
	c=0
	#no cerrastes la etiqueta o no la abristes antes de cerrar
	if len(aperturas)!=len(cierres):
		if len(aperturas)>len(cierres):
			for k,elem in enumerate(aperturas):
				#si abre dos veces
				if k<len(aperturas)-2:
					if cadena[aperturas[k]:aperturas[k+1]].count(etiquetas[0])!=cadena[aperturas[k]:aperturas[k+1]].count(etiquetas[1]):
						print "No se cerro las etiqueta en: ",cadena[aperturas[k]:aperturas[k+1]]
						raise
		else:
			for k,elem in enumerate(cierres):
				#si abre dos veces
				if k<len(cierres)-2:
					if cadena[cierres[k]:cierres[k+1]].count(etiquetas[0])!=cadena[cierres[k]:cierres[k+1]].count(etiquetas[1]):
						print "No se cerro las etiqueta en: ",cadena[cierres[k]:cierres[k+1]]
						raise
...
def getConjuntos2(cadena,etiquetas,exclusion=None,c=0,sigue=[" ","]"]):
	aperturas=[]
	cierres=[]
	c=c
	activa=False
	...
	while c+len(etiquetas[1])<=len(cadena):
		...
		if activa==False:
			if type(etiquetas[0])==str:
				...
				if cadena[c:c+len(etiquetas[0])]==etiquetas[0] and cadena[c+len(etiquetas[0])] in sigue  and abierto==False:
					if c not in aperturas:
						aperturas.append(c)
						cierres.append(c+len(etiquetas[0]))
					abierto=True
				elif cadena[c:c+len(etiquetas[1])]==etiquetas[1] and cadena[c+len(etiquetas[1])] in sigue and abierto==True:
					if len(cierres)<=len(aperturas):
						if c not in aperturas:
							aperturas.append(c)
							cierres.append(c+len(etiquetas[1]))
				...
				for elem in etiquetas:#cuando son varias etiquetas
	...
	if exclusion==None:
		...
		for elem in ex_aper:
			exclusiones.append([elem,ex_cier[c]])
			c+=1
		conjuntos=borrar2([aperturas[0],cierres[0]],exclusiones)
