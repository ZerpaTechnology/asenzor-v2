...
class nav():
		def __init__(self):
			...
			self.path=os.getcwd()
			self.list_current=self.path.split("/")
			self.pos=len(self.list_current)

		def nav(self,direccion):
			x=""
			pos=len(self.list_current)

			if direccion==">":
				if self.pos<len(self.list_current):
					self.pos+=1
				c=0
				while c<self.pos:
					x+=self.list_current[c]+"/"
					c+=1
				self.path=x
				...
				if self.pos<=len(self.list_current) and self.pos>0:
					self.pos-=1
				c=0
				while c<=self.pos:
					x+=self.list_current[c]+"/"
			...
			elif "/" == direccion[-1] and "/" != direccion[0]:
				if direccion[1:] in os.listdir():
					self.list_current.append(direccion)
