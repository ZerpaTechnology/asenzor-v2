...
def DB(dbfile=None,debug=False):
        ...
        def db(tabla=None,herencia=None,copia=None):
			    self=db
			    self.t=tabla
			    self.error=[]

					
			    def consola(mensaje,d,error=False):
					d.log.append(mensaje)
					if d.debug==True:
						print mensaje
					if error:
						d.errores.append(mensaje)
        ...
        if dbfile==None:
			...
			if str(type(x))!="<type 'function'>":
				try:
					import traceback
					import sys
					exc_type,exc_obj,exc_tb=sys.exc_info()
					fname = exc_tb.tb_frame.f_code.co_filename
					db.log.append("Ocurrio un error al cargar la base de datos "+dbfile+"\n"+str(x)+"\n"+"".join(traceback.format_exception(exc_type,fname,exc_tb)))
					db.errores.append("Ocurrio un error al cargar la base de datos "+dbfile+"\n"+str(x)+"\n"+"".join(traceback.format_exception(exc_type,fname,exc_tb)))
				except:
					db.log.append("Ocurrio un error al cargar la base de datos "+dbfile+"\n"+str(x))
					db.errores.append("Ocurrio un error al cargar la base de datos "+dbfile+"\n"+str(x)+"\n"+"".join(traceback.format_exception(exc_type,fname,exc_tb)))
