...
def treeFolder(path):
	...
	for f in os.listdir(path):
		...
		if os.path.isdir(path+f):
			...
			carpetas.append({f:treeFolder(path+f)})
		else:

			archivos.append(f)
	carpetas.extend(archivos)
	return carpetas

def rstr(cadena,sub,vez):
	cadena2=""
	c=len(cadena)-1
	while c>=0:
		cadena2=cadena[c]+cadena2
...
class redict:
	def __init__(self,diccionario):
		"""
		Es un diccionario especial de python
		que soporta claves con regex
		"""
		self.diccionario=dict(diccionario)
	def __getitem__(self,clave):
		import re
		for pattern in list(self.diccionario):
			if re.search(pattern, clave):
				return self.diccionario[pattern]
...
def reKeyIn(patron,lista):
	if "*" not in patron:
		if "[" in patron:
			pass
		

def getPattern(lista):
	"""
	Funcion que intenta deducir un patron de regex entre
	los items de la lista pasada como parametro
	"""
	exp1=[]#tipo:[a|o]
	similitudes=[]
	previo=None
		...
		if previo!=None:
			...
			for k,ch in enumerate(previo):
				if ch==elem[k]:
					...
					if k+2<len(previo):
						"""
						Esto quiere decir que si previo es por ejemplo:
						
						      k=6 len=8 len-k=2
						01234567
						muchacho

						y si k+2<len es decir pasa el margen de 2 caracteres para casos como
						femenino masculino y plural [a,o,as,os] entonces posiblemente le siga otra constante
						y lo que esta en el medio es una expresion regular
						"""
						temp=""
						for k2, ch2 in enumerate(previo):
							if ch2!=elem[k2]:
								temp+=elem[k2]
							else:
								if elem[k:] not in exp1:
									exp1.append(temp)
								break
...
def iterRelacionado():
	"""
	Esta funcion permite hacer las modificaciones directamente en el 
	elemento iterado y no crea elemento copia es decir el poder hacer esto
	lista=[1,2,3,4]
	for elem in lista:
		elem(elem+1)
	[2,3,4,5]
...
def filtrar_datos_planos(data):
	...
	if type(data)==dict:
		...
		return data
def filtrar_del_dict(diccionario,keys):
	d={}
	for elem in keys:
...
def update(k,v):
	if type(k)==list and type(v)==list:
	     ...
	     for elem in v:
	             k.append(elem)
	elif type(k)==dict and type(v)==dict:
		while list(k):
			del k[list(k)[0]]
		for elem in v:
	             k[elem]=v[elem]
