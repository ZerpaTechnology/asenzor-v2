"use strict";
// Transcrypt'ed from Python, 2018-01-20 18:02:44
function Builder () {
				...
				__init__: function (__all__) {
					...
					var Column = __class__ ('Column', [Widget], {
						...
						get add () {return __get__ (this, function (self, widget) {
							...
							self.target.find ('>.content').find ('>.add').before (widget.target);
						});},
						get py_update () {return __get__ (this, function (self) {
							self.format = list ([self._titulo]);
							self.__update__ ();
				...
				__init__: function (__all__) {
					...
					var InsertModulo = __class__ ('InsertModulo', [Widget], {
						...
						get __init__ () {return __get__ (this, function (self, titulo) {
							...
							self.__button = self.target.find ('>button');
							self.BasicTabs = BasicTabs ();
							self._modulos = list ([BuilderModulo]);
						});},
						get titulo () {return __get__ (this, function (self, titulo) {
							self.target.find ('.titulo').text (titulo);
							self._titulo = titulo;
		...
		'_Builder.Row', {
			__all__: {
				...
				__init__: function (__all__) {
					...
					var Widget = __init__ (__world__.Widget).Widget;
					var Column = __init__ (__world__.Column).Column;
				...
				__init__: function (__all__) {
					...
					var Window = __class__ ('Window', [Widget], {
						...
						get __init__ () {return __get__ (this, function (self, titulo) {
							...
							self.target.html (self._html);
							self.__button = self.target.find ('>button');
							self._btns = list ([list (['Nuevo Modulo', 'save', (function __lambda__ (self) {
						...
						get open () {return __get__ (this, function (self, ventana) {
							...
							if (ventana == 'InsertColumns') {
								print (dir (self.InsertColumns));
								self.InsertColumns.py_update ();
								self.target.find ('>.content').html (self.InsertColumns.target);
							}
							else if (ventana == 'InsertModulo') {
								self.InsertModulo.py_update ();
								self.target.find ('>.content').html (self.InsertModulo.target);
							}
							self.reload ();
						});},
						...
						get py_update () {return __get__ (this, function (self) {
							...
							self.__update__ ();
							self.titulo (self._titulo);
							self.__btns = self.target.find ('>.header').find ('>.btns').find ('>.btns-left');
	...
	(function () {
		...
		var Builder = __class__ ('Builder', [Widget], {
			...
			get appendRow () {return __get__ (this, function (self) {
				...
				row.py_update ();
				self.target.find ('>.content').find ('>.add').before (row.target);
			});},
			...
			get py_update () {return __get__ (this, function (self) {
				...
				self.titulo (self._titulo);
				self.target.find ('>.window').html (self.window.target);
				self.window.hidden ();
				self.__titulo = self.target.find ('>.titulo');
			});}
		});
