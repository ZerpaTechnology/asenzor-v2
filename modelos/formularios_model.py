	...
	class model(Model):
		...
		def crearFormulario(self,data):
			if self.request():
				...
				for k2,box in enumerate(self.obtenerFilas("Formularios")[plantilla][1]):
				...
				self.grabar()
		def registrarFormulario(self,indice,plantilla,data,tabla="Post-de-Formulario"):
			if self.request():
				select=True
				l2=list(data)
				...
				from modulos.ztec.zred import urlBuilder
				
				
				mensaje="<p>Un visitante a llenado el formulario de contacto puedes verlo en <a href='"+urlBuilder(config,settings.app,"admin","index","PostdeFormulario",i,action="editar")+"'>visitando tu sitio</a> o mendiante este email:</p>"

				for k2,box in enumerate(self.obtenerFilas(plantilla)[indice][1]):
				...
				boxes.insert(0,{"Formulario":"hidden-id","name":"id","value":indice})
