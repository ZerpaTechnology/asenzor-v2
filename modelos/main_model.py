...
try:
	class model(Model):
		...
		def obtenerMenus(self,menu=None):
			if self.update():
				menus=[]
				def formatear(menu):
					children=[]
					...
					if len(menu["value"])==3:
						...
						url=menu["url"]
					mimenu={menu["name"]:url,"children":[]}
					for elem in menu["children"]:
						children.append(formatear(menu))
					mimenu["children"]=children
					return mimenu


				for m in self.obtenerFilas("Menus"):
					if menu!=None:
						return formatear(m[1])
					else:
						menus.append(formatear(m[1]))
		...
		def activarPlugin(self,plugin):#activa/desactiva
			if self.request():
				for k,elem in enumerate(self.obtenerFilas("Plugins")):
					if elem[0]==plugin:
						self.db("Plugins").modificarCampo(k,'Activo', False if elem[1]==True else True)
						return self.grabar()
