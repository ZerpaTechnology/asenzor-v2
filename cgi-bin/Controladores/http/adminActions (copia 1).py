          ...
          if "args" in p:
            ...
            if i!=None:
              ...
              if keys=="Menu":
                ...
                data["item"]=data["model"]["main"].obtenerFilas("Menus")[i][0]
                ...
                l1=[]             
                data["titulo"]=data["model"]["paginas"].obtenerFilas("Entradas")[i][0]
                data["item"]=data["model"]["paginas"].obtenerFilas("Entradas")[i]
                
                l1=data["model"]["paginas"].obtenerFilas("Entradas")[i][1]
                data["boxes"]=l1
              elif keys=="Archivo":
                l1=[]
                l2=[]
                ...
                data["boxes"]=l1
              elif keys=="globalUser":
                l1=[]

                data["titulo"]=data["model"]["global"].obtenerFilas("Usuarios")[i][0]
                #data["model"]["main"].obtenerFilas("Paginas")[i][1]
                l1=data["model"]["global"].obtenerFilas("Usuarios")[i][1]

                


                if data["isGlobalUser"]:
              ...
              if keys=="Archivo":
                ...
                data["titulo"]=keys
                #data["model"]["main"].obtenerFilas("Paginas")[i][1]
                for box in data["model"]["global"].obtenerFilas("Usuarios")[0][1]:
                 l=[]
                 for campos in box:
                  if type(campos)==dict:
                   campos["value"]=""
                ...
                data["boxes"]=[
                [{'Titulo': 'text', 'name': 'titulo', 'value': ''}, 
                {'Contenido': 'textarea', 'name': 'publish', 'value': ''}]]
              elif keys=="Info":
                l1=[]
                i=normalizar(p["args"][1])
                data["titulo"]=data["model"]["main"].obtenerFilas("AdminMenu")[3][2]["titulo"]
                ...
                #data["model"]["main"].obtenerFilas("Paginas")[i][1]
                for box in data["model"]["paginas"].obtenerFilas("Entradas")[0][1]:
                 l=[]
                 for campos in box:
                  if type(campos)==dict:
                   campos["value"]=""
