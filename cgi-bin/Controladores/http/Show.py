	...
	def Show(self):
		...
		if self.data["login"]:
		    ...
		    if list(normalizar(self.data["kwargs"]))==["widget"]:
		      import os
		      if self.data["kwargs"]["widget"].strip()+".html" in os.listdir(config.base_root+(config.apps_dir+"/" if self.data["manager"]==False else config.projects_folder)+settings.app+"/user/"+routes.vistas_folder+routes.widgets_folder):
		        f=open(config.base_root+(config.apps_dir+"/" if self.data["manager"]==False else config.projects_folder)+settings.app+"/user/"+routes.vistas_folder+routes.widgets_folder+self.data["kwargs"]["widget"].strip()+".html","r")
		        print f.read()
		        f.close()
		      else:
		        print None
