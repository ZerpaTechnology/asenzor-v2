  ...
  if p["control"]=="admin"  and "vista" in p: 
    ...
    if errores==[]:
      ...
      if  data["login"]==True:
       ...
       if p["vista"]=="pagNoticias":
       ...
       elif p["vista"]=="index" or p["vista"]==None:    
        ...
        data["opciones"]={"main":data["model"]["main"].obtenerFilas("Opciones"),
                         ...
                         "informaciones":data["model"]["informaciones"].obtenerFilas("Opciones"),
                         "global":data["model"]["global"].obtenerFilas("Opciones"),
                        }
        ...
        if p["control"]=="admin":
         ...
         if p["isGlobalUser"]:
          data["user"]=data["model"]["global"].getUser(p["token"])
         else:
         ...
         for elem in data["adminMenu"]:
          
          if len(elem[3])>1 and "titulo" in data and elem[3][0][1][1]["valor"]==data["titulo"]:


            if len(elem[3][1][1][1]["valor"])>1 and (type(elem[3][1][1][1]["valor"])==list or type(elem[3][1][1][1]["valor"])==dict or type(elem[3][1][1][1]["valor"])==tuple):
              
              data["addNew-enlace"]=zred.urlBuilder(config,settings.app,"admin","index",elem[3][1][1][1]["valor"][0],str(elem[3][1][1][1]["valor"][1]),action=elem[3][1][1][0])
         ...
         data["creditos"]=p["model"]["global"].obtenerCreditos(p["app"])[p["app"]]

         servir("index",data,isglobal=True)
       ...
       servir("login",data,isglobal=True)
    ...
    if errores==[]:
      ...
      if p["vista"]=="index":
       ...
       data["actionbase"]="app="+settings.app+"&vista="
        ...
        if "vistas" not in settings.get_controllers:
          servir("index",data)
        ...
        data["opciones"]={"main":data["model"]["main"].obtenerFilas("Opciones"),
                         ...
                         "formularios":data["model"]["formularios"].obtenerFilas("Opciones"),
                         "informaciones":data["model"]["informaciones"].obtenerFilas("Opciones"),
                         "publicaciones":data["model"]["publicaciones"].obtenerFilas("Opciones"),
                        }
        def _min(foto):
          return foto[:foto.find(".")]+"_540x540"+foto[foto.find("."):]
        data["min"]=_min
