...
(function(){
	!function(e,t,n){
		...
		return p=/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent),
			   r=/Chrome/.test(navigator.userAgent),
			   u=/Chrome/.test(navigator.userAgent)&&/Google Inc/.test(navigator.vendor)||/Safari/.test(navigator.userAgent)&&/Apple Computer/.test(navigator.vendor),
			   a=p&&/crios/i.test(navigator.userAgent),
			   h=/iPhone|iPad|iPod/i.test(navigator.userAgent)&&!!navigator.userAgent.match(/Version\/[\d\.]+.*Safari/),
			   f=navigator.appVersion.indexOf("MSIE")!==-1||navigator.appVersion.indexOf("Trident/")>-1,
			   d=/windows nt 6.2/.test(navigator.userAgent.toLowerCase())||/windows nt 6.3/.test(navigator.userAgent.toLowerCase()),
			   s=null!=t.body.classList,
			   o=!!r&&navigator.userAgent.replace(/^.*Chrome\/([\d\.]+).*$/i,"$1"),
			   l=o>="55.0.2883.75",
			   i=function(){
			   	function i(t){
			   		this.options=e.extend(!0,{},this.Defaults,t),
			   		this.scenes=[],this.initialize(),
			   		this.scrollY=n.scrollY||n.pageYOffset,
			   		this.lastScrollY=-1,
			   		this.lastDocHeight=0,
			   		this.inputFocus=!1,
			   		this.checkLayerHeight=!1
			   	}
			   	var o,
			   	g;
			   	return o=function(){
			   		function e(e,n,i,o,s,r,a){
			   			switch(this.amend=u||f||p?l?0:60:0,
			   				this.element=e,
			   				this.aliases=n,
			   				this.type=e.getAttribute("data-type")||"html",
			   				"html"===this.type&&(this.holder=this.createHolder()),
			   				this.direction="normal"===e.getAttribute("data-direction")||null==e.getAttribute("data-direction")?1:-1,
			   				this.fade="true"===e.getAttribute("data-fade"),
			   				this.blur="true"===e.getAttribute("data-blur"),
			   				this.boundTo=t.querySelector(e.getAttribute("data-bound-to")),
			   				"media"===this.type&&(this.url=e.getAttribute("data-url")),
			   				this.responsive=this.getResponsiveOptions(),
			   				!f&&!p||p||d&&f?this.element.style.position="absolute":this.element.style.position="fixed",
			   				this.type)
			   			{
			   				case"media":null!=this.url&&(this.element.style["background-image"]="url("+this.url+")");
			   				break;
			   				case"html":f&&p&&(this.element.style["z-index"]=1)
			   			}
			   		...
			   		return e.prototype.refresh=function(e,t,n,i,o){
			   			var s;
			   			switch(s=this,
			   				s.speed=s.getOption("speed",e)||0,
			   				s.offset=s.getOption("offset",e)||0,
			   				p||d&&f||(o?s.element.style.position="fixed":s.element.style.position="absolute"),
			   				f&&"html"===s.type&&(s.element.style.position="absolute"),
			   				s.type){
			   				case"media":if(!f)return s.offsetHeight=s.getMediaHeight(t,i,s.speed,s.direction),
			   			s.element.style.height=s.offsetHeight+"px";
			   			break;
			   			case"html":if(s.element.style.width=this.holder.offsetWidth+"px",
			   				s.offsetHeight=s.element.offsetHeight,
			   				s.holder.style.height=s.offsetHeight+"px",
			   				!(!f&&!p||p||d&&f))
			   			return f?s.element.style.position="static":o&&(s.element.style.left=s.getOffset(s.holder).left+"px",
			   				s.element.style.top=s.getOffset(s.holder).top-n+"px"),
			   			s.holder.style.position="static";
			   			break;
			   			case"custom":return s.offsetHeight=s.element.offsetHeight}},
			   			e.prototype.createHolder=function(){
			   				var e,n;
			   				return n=this,
			   				e=t.createElement("div"),
			   				s?e.classList.add("rd-parallax-layer-holder"):e.className="rd-parallax-layer-holder",
			   				n.element.parentNode.insertBefore(e,n.element),
			   				e.appendChild(n.element),
			   				(!f&&!p||a)&&(e.style.position="relative"),
			   				e},
			   				e.prototype.isHolderWrong=function(){
			   					...
			   					e.prototype.getResponsiveOptions=function(){
			   						...
			   						for(n in a)e=a[n],l.push(n),t.push(e);
			   							for(n=o=0,s=l.length;o<s;n=++o)for(r=l[n],f[r]={};
			   								(i=n)>=-1;
			   								)!f[r].speed&&(p=this.element.getAttribute("data"+t[i]+"speed"))&&(f[r].speed=this.getSpeed(p)),
			   								!f[r].offset&&(p=this.element.getAttribute("data"+t[i]+"offset"))&&(f[r].offset=parseInt(p)),
			   							!f[r].fade&&(p=this.element.getAttribute("data"+t[i]+"fade"))&&(f[r].fade="true"===p),n--;
			   							return f},
			   							e.prototype.fuse=function(e,t){
			   								var n,i,o,s,r,a;
			   								n=this,o=n.getOffset(n.element).top+n.element.getBoundingClientRect().top,
			   								a=e+t/2,i=o+n.offsetHeight/2,
			   								r=t/6,a+r>i&&a-r<i?n.element.style.opacity=1:(s=a-r<i?1+(a+r-i)/t/3*10:1-(a-r-i)/t/3*10,
			   									n.element.style.opacity=s<0?0:s>1?1:s.toFixed(2))},
			   								e.prototype.move=function(e,t,n,i,o,s,r,l,h){
			   									var g,c,m,y,v;
			   									if(m=this,!(f&&"media"===m.type||p||d&&f)){
			   										if(!r)return u&&(m.element.style["-webkit-transform"]="translate3d(0,0,0)"),
			   											void(m.element.style.transform="translate3d(0,0,0)");
			   										v=!p||"html"===m.type&&h||a?m.speed*m.direction:m.speed*m.direction-1,
			   										c=m.offsetHeight,
			   										null!=l?g=(i+n-(l+n))/(n-o):"media"!==m.type&&(i<n||i>s-n)?(g=i<n?i/(n-o):(i+n-s)/(n-o),
			   											isFinite(g)||(g=0)):g=.5,y=a||f?(o-c)/2+(n-o)*g*v+m.offset:p?-(i-e)*v+(o-c)/2+(n-o)*g*(v+1)+m.offset:-(i-e)*v+(o-c)/2+(n-o)*g*v+m.offset,
			   										p&&null!=l&&(m.element.style.top=i-l+"px"),

			   										u&&(this.element.style["-webkit-transform"]="translate3d(0,"+y+"px,0)"),
